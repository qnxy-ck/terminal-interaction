...
public class SwipeCardMessageProcessor implements ClientMessageProcessor<SwipeCard> {
    ...
    private AuthorizedMoveOutGoodsService authorizedMoveOutGoodsService;
    ...
    public Mono<Void> handle(SwipeCard swipeCard, Client client) {
        return Mono.deferContextual(ctx -> {
            ...
            // 调用外部刷卡接口
            return this.authorizedMoveOutGoodsService.swipeCard(terminalId, swipeCard.cardCode())
                    .onErrorResume(e -> {
                    ...
                    .flatMap(it -> CallingFlow.authorizedMoveOutGoods(
                                    ...
                                    this.authorizedMoveOutGoodsService
                            )
                    );
        });
    }
